...
int main() {
    ...
    Persona personas[MAX_TAM] = {},
            alergicos[MAX_TAM] = {}, noAlergicos[MAX_TAM] = {};
    int tamReal = 0, numAlergicos = 0, numNoAlergicos = 0;
    char entrada;
    do{
        cout << "Dime el nombre: " << endl;
        getline(cin>>ws,personas[tamReal].nombre);
        do{
            cout << "Es alergico?(s/n): " << endl;
            cin >> entrada;
        }while(entrada != 's' && entrada != 'n');
        if(entrada == 's') personas[tamReal].esAlergico = true;
        else personas[tamReal].esAlergico = false;
        ++ tamReal;
        do{
